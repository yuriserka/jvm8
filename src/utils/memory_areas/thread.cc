...
namespace MemoryAreas {
    void Thread::executeMethod(const std::string &method_name,
        ...
        for (auto it = code_array.begin(); it != code_array.end(); ++it) {
            ...
            //achar exception table
            exception_table;
            
            
            for(auto it = exception_table.begin(); it != exception_table.end(); it++){
                auto elem_class_name = it->class_name; //sintaxe esta errada
                if(elem_class_name == obj_class_name){
                    //tratar excecao
                }
            }
