...
void printBytecode(std::vector<Utils::Types::u1>::iterator *code_it, Viewer *v,
  ...
  switch (opcode) {
    ...
    case Opcodes::kANEWARRAY: {
      ...
      auto args = i->toBytecode(code_it, &delta_code, wide);
      std::wcout << "<" << v->getConstantPoolInfo(args[0], false) << ">\n";
      break;
    }
    ...
    case Opcodes::kCHECKCAST: {
      ...
      auto args = i->toBytecode(code_it, &delta_code, wide);
      std::wcout << "<" << v->getConstantPoolInfo(args[0]) << ">\n";
      break;
    }
    case Opcodes::kD2F: {
    ...
    case Opcodes::kGOTO: {
      ...
      std::cout << static_cast<int>(char(args[0] + *code_index)) << " ("
    ...
    case Opcodes::kINVOKEINTERFACE: {
      ...
      auto args = i->toBytecode(code_it, &delta_code, wide);
      std::wcout << "<" << v->getConstantPoolInfo(args[0]) << "> count "
                 << args[1] << "\n";
      break;
    }
